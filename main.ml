...
let (* rec? *) matchesHiOrHelloStar : string -> bool =
  fun str ->
    ...
    in  
    let rec checkHi chars =
    (
    match chars with 
    | 'H'::'i'::rest -> checkHi rest
    | [] -> true
    | _ -> false 
    )
    in
     ...
     let matchesHiOrHelloOkayStar : string -> bool =
      fun str ->
        let rec checkOnlyOkay chars =  
          ...
          | _ -> false      
        in
        let rec checkHiOkay1 chars =
          match chars with
          | 'H'::'i'::rest -> checkHiOkay1 rest
          | 'H'::'i'::'O'::'k'::'a'::'y'::rest -> checkOnlyOkay rest 
          | [] -> true
          | _ -> false
        in 
        let rec checkHelloOkay1 chars =
          match chars with
          | 'H'::'e'::'l'::'l'::'o':: rest -> checkHelloOkay1 rest 
          | 'H'::'e'::'l'::'l'::'o'::'O'::'k'::'a'::'y'::rest -> checkOnlyOkay rest  
          | [] -> true
          | _ -> false 
        in 
